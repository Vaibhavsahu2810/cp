 ...
 * @brief Call SPDX agent to generate report from UI
 */
class SpdxGeneratorUi extends DefaultPlugin
{
    const NAME = 'ui_spdx';                ///< Mod name of the plugin
    const DEFAULT_OUTPUT_FORMAT = "spdx";  ///< Default report format
    ...
    function preInstall()
    {
        ...
        menu_insert("Browse-Pfile::Export&nbsp;SPDX&nbsp;RDF&nbsp;report", 0, self::NAME . '&outputFormat=spdx', $text);
        menu_insert("UploadMulti::Generate&nbsp;SPDX", 0, self::NAME, $text);

        $text = _("Generate SPDX report in tag:value format");
        menu_insert("Browse-Pfile::Export&nbsp;SPDX&nbsp;tag:value&nbsp;report", 0, self::NAME . '&outputFormat=spdxtv', $text);

        $text = _("Generate CSV report (with SPDX IDs)");
        menu_insert("Browse-Pfile::Export&nbsp;CSV&nbsp;report&nbsp;(SPDX)", 0, self::NAME . '&outputFormat=spdxcsv', $text);
    ...
    protected function getJobAndJobqueue($groupId, $upload, $addUploads)
    {
        ...
        $spdxAgent = plugin_find('agent_'.$this->outputFormat);
        ...
        $params = array($spdxAgent->AgentName,$uploadId,$userId,$groupId);
        $log = __METHOD__;
        if ($jqCmdArgs) {
        ...
        $error = "";
        $jobQueueId = $spdxAgent->AgentAdd($jobId, $uploadId, $error, array(), $jqCmdArgs);
        if ($jobQueueId < 0) {
...
if (!class_exists('Fossology\Spdx\UI\SpdxGeneratorUi')) {
    register_plugin(new SpdxGeneratorUi());
}