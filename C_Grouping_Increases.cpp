...
void solve()
{
    ...
    for (ll i = 1; i < n; i++)
    {
        if(ar1.size() > 0 && ar2.size() > 0){
            ...
            else if(a[i] <= ar1.back()){
                ar1.pb(a[i]);
            }
            else if (a[i] <= ar2.back())
            {
                ar2.pb(a[i]);
            }
            else{
...
public:
    int rob(TreeNode *root)
    {
        function<pair<int, int>(TreeNode *)> dfs = [&](TreeNode *node) -> pair<int, int>
        {
            ...
            auto [lwr, lor] = dfs(node->left);
            auto [rwr, ror] = dfs(node->right);

            int wr = node->val + lor + ror;
            int orr = max(lwr, lor) + max(rwr, ror);
